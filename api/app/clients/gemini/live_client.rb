...
module Gemini
    ...
    class LiveClient
        ...
        def check_silence_pump_ready
            if @silence_pumping || !@connected || !@ws || @superseded || !@last_real_audio_at || @model_emitting_audio
                return
            end
        ...
        def log_gemini_event(data)
            ...
            if sc
                ...
                if has_audio && !has_input_tx && !has_output_tx && !turn_complete && !gen_complete && !interrupted
                    return
                end
                ...
                if has_input_tx
                    input_tx_text = sc.dig('inputTranscription', 'parts', 0,
                                           'text') || sc.dig('inputTranscription', 'text')
                    parts << "inputTx=#{input_tx_text.truncate(50)}" if input_tx_text.present?
                end
                if has_output_tx
                    output_tx_text = sc.dig('outputTranscription', 'parts', 0,
                                            'text') || sc.dig('outputTranscription', 'text')
